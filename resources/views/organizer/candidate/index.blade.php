            ...
            <div class="row">
                                ...
                                <div class="table-responsive">
                                    <table id="example1"
                                        ...
                                        @foreach($candidates as $event)
                                            <tr>
                                                <th scope=row>{{$i++}}</th>
                                                <td>{{$event->post->post_name}}</td>
                                                <td>{{$event->candidate_name}}</td>
                                                <td class="text-right">

                                                    <a href="{{route('candidates.edit',[$event->id])}}"
                                                    ...
                                                    {!! Form::open(['method' => 'DELETE', 'class'=>'inline', 'route'=>['candidates.destroy',
                                                        $event->id]]) !!}
                                                    <button type="submit"
                ...
                <div class="col-md-3">
                    @if(\Request::segment(4)=='edit')
                        @include('organizer.candidate.edit')
                    @else
                        @include('organizer.candidate.add')
                    @endif
